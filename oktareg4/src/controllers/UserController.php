...
class UserController
{
    ...
    public function handleRegistrationPost()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            header('Location: /?thankyou change landings page in src/controller/nned to sent to sign in page');
            return true;
        }
    ...
    private function validateRegistrationForm($input)
    {
		...
		if (strlen($input['mobile_phone']) > 100) {
            $errorMessage .= "<br>'Mobie Phone' is too long (100 characters max)!";
            $errors = true;            
        }
		...
		if (empty($input['mobile_phone'])) {
            $errorMessage .= "<br>'Mobile Phone' is required!";
            $errors = true;
        }
